    ...
    LazyColumn(modifier = modifier.fillMaxSize().background(PwBg)) {
        ...
        item {
            ...
            Row(
                ...
                Row(verticalAlignment = Alignment.Bottom, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    Box(
                        modifier = Modifier.clip(RoundedCornerShape(5.dp)).background(accent).padding(horizontal = 6.dp, vertical = 2.dp)
                    ) {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(9.dp), verticalAlignment = Alignment.CenterVertically) {
                    Box(
                        modifier = Modifier.size(34.dp).border(1.dp, Color.White.copy(alpha = 0.12f), CircleShape),
        ...
        item {
            ...
            Text(
                ...
                color = PwTextDim, letterSpacing = 1.6.sp
            )
            LazyRow(
                    ...
                    Column(Modifier.padding(horizontal = 18.dp, vertical = 15.dp)) {
                        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                            Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                                PulseDot(accent)
                                Text("NEXT RACE · ROUND 11", fontSize = 10.sp, fontWeight = FontWeight.Bold, color = accent, letterSpacing = 1.4.sp)
                            }
                            Text("GBR", fontFamily = FontFamily.Monospace, fontSize = 11.sp, fontWeight = FontWeight.SemiBold, color = PwTextMuted)
                        }
                        Text("BRITISH\nGRAND PRIX", fontWeight = FontWeight.Black, fontSize = 28.sp, lineHeight = 27.sp, letterSpacing = (-0.6).sp, color = PwText, modifier = Modifier.padding(top = 11.dp))
                        Text("Silverstone Circuit · 52 laps · 5.891 km", fontSize = 12.5.sp, color = PwTextSub, modifier = Modifier.padding(top = 6.dp))
                        Row(Modifier.padding(top = 16.dp).fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(7.dp)) {
                            listOf(pad(cdD) to "DAYS", pad(cdH) to "HRS", pad(cdM) to "MIN", pad(cdS) to "SEC").forEachIndexed { i, (v, label) ->
                        ...
                        Row(Modifier.fillMaxWidth().padding(top = 14.dp), horizontalArrangement = Arrangement.SpaceBetween) {
                            Text("Lights out", fontSize = 12.sp, color = PwTextMuted)
        ...
        item {
            // ── Title Fight ─────────────────────────────────────────────────────
            SectionHeader("TITLE FIGHT", "Standings ›", accent)
            Box(
                modifier = Modifier
                    .padding(horizontal = 14.dp)
                    ...
                    top3.forEachIndexed { i, d ->
                        ...
                        Column {
                            ...
                            Row(Modifier.fillMaxWidth().padding(horizontal = 15.dp, vertical = 13.dp), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                                Text("${d.pts}", fontFamily = FontFamily.Monospace, fontWeight = FontWeight.Bold, fontSize = 15.sp, color = PwTextMuted, modifier = Modifier.width(18.dp))
                                Box(Modifier.width(4.dp).height(30.dp).clip(RoundedCornerShape(2.dp)).background(teamColor))
                                Column(Modifier.weight(1f)) {
                                    Text("${d.first} ${d.last}", fontWeight = FontWeight.Bold, fontSize = 14.5.sp, color = PwText)
                                    Text(teamMap[d.teamId]?.name ?: "", fontSize = 11.sp, color = PwTextMuted)
                                }
                                Column(horizontalAlignment = Alignment.End) {
                                    Text("${d.pts}", fontFamily = FontFamily.Monospace, fontWeight = FontWeight.Bold, fontSize = 16.sp, color = PwText)
                                    Text("PTS", fontSize = 9.sp, color = PwTextDim, letterSpacing = 0.5.sp)
                                }
                            }
                        }
                    }
        ...
        item {
            // ── My Team ─────────────────────────────────────────────────────────
            Text("MY TEAM", modifier = Modifier.padding(start = 18.dp, bottom = 10.dp), fontWeight = FontWeight.ExtraBold, fontSize = 15.sp, color = PwText)
            Box(
                modifier = Modifier
                    .padding(horizontal = 14.dp)
                    .clip(RoundedCornerShape(18.dp))
                    .background(Brush.linearGradient(listOf(accent.copy(alpha = 0.15f), Color.Transparent)))
                    .border(1.dp, accent.copy(alpha = 0.35f), RoundedCornerShape(18.dp))
            ) {
                Column(Modifier.padding(horizontal = 18.dp, vertical = 16.dp)) {
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                        Column {
                            Text(selectedTeam.name, fontWeight = FontWeight.Black, fontSize = 21.sp, color = accent, letterSpacing = (-0.4).sp)
                            Text(selectedTeam.full, fontSize = 11.5.sp, color = PwTextSub)
                        }
                        Column(horizontalAlignment = Alignment.End) {
                            Text("P$constrPos", fontFamily = FontFamily.Monospace, fontWeight = FontWeight.Bold, fontSize = 15.sp, color = PwText)
                            Text("CONSTRUCTORS", fontSize = 9.sp, color = PwTextDim, letterSpacing = 0.5.sp)
                        }
                    }
                        ...
                        myDrivers.take(2).forEach { d ->
                            Column(
                                modifier = Modifier.weight(1f).clip(RoundedCornerShape(12.dp))
                                    .background(Color.White.copy(alpha = 0.04f)).padding(11.dp)
                            ) {
                                Text("#${d.number}", fontFamily = FontFamily.Monospace, fontWeight = FontWeight.Bold, fontSize = 12.sp, color = accent)
                                Text(d.last, fontWeight = FontWeight.Bold, fontSize = 14.sp, color = PwText, modifier = Modifier.padding(top = 4.dp))
                                Text("P${allDrivers.indexOfFirst { it.id == d.id } + 1} · ${d.pts} pts", fontSize = 11.sp, color = PwTextSub, modifier = Modifier.padding(top = 1.dp))
                            }
                        }
                    }
                }
        ...
        item {
            // ── Latest News ─────────────────────────────────────────────────────
            SectionHeader("LATEST", "News ›", accent)
                    ...
                    Row(
                        ...
                        Box(
                            modifier = Modifier.size(54.dp).clip(RoundedCornerShape(9.dp))
                                .background(Brush.linearGradient(135f, listOf(Color(0xFF22222A), Color(0xFF191920))))
                        )
                        Column(Modifier.weight(1f)) {
                            Text(n.category, fontSize = 9.sp, fontWeight = FontWeight.Bold, color = accent, letterSpacing = 1.sp)
                            Text(n.title, fontWeight = FontWeight.SemiBold, fontSize = 12.5.sp, lineHeight = 16.sp, color = PwText, modifier = Modifier.padding(top = 3.dp))
...
private fun SectionHeader(title: String, action: String, accent: Color) {
    Row(
        modifier = Modifier.fillMaxWidth().padding(horizontal = 18.dp, vertical = 0.dp).padding(bottom = 10.dp),
        horizontalArrangement = Arrangement.SpaceBetween,
        verticalAlignment = Alignment.CenterVertically
    ) {
